...
docker build -t metorikku/spark:k8s --build-arg SPARK_VERSION=$SPARK_VERSION -f docker/spark/k8s/Dockerfile docker/spark/k8s
...
docker build -t metorikku/spark:spark2_k8s --build-arg SPARK_VERSION=$SPARK2_VERSION -f docker/spark/k8s/Dockerfile docker/spark/k8s
...
docker build -t metorikku/spark:spark2_standalone -f --build-arg IMAGE_NAME=metorikku/spark:spark2_hadoop docker/spark/standalone/Dockerfile docker/spark/standalone
docker build -t metorikku/metorikku:spark2_standalone --build-arg IMAGE_NAME=metorikku/spark:spark2_standalone -f docker/metorikku/Dockerfile .
docker build -t metorikku/metorikku:spark2_k8s --build-arg IMAGE_NAME=metorikku/spark:spark2_k8s --build-arg SCALA_VERSION=${SPARK2_SCALA_MAJOR_VERSION} -f docker/metorikku/Dockerfile .
