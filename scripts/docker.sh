...
docker pull $(grep -ioP '(?<=^from)\s+\S+' docker/k8s-spark-operator/Dockerfile)
...
docker build -t metorikku/k8s-spark-operator --cache-from metorikku/k8s-spark-operator --build-arg SPARK_VERSION=2.4.5 --build-arg HADOOP_VERSION=2.9.2 -f docker/k8s-spark-operator/Dockerfile docker/k8s-spark-operator
