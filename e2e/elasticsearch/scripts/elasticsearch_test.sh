...
queryCount=$(runCountQuery)
num_retries=0
until [[ $(echo "queryCount") = ${num_of_expected_rows} || num_retries = ${MAX_RETRIES} ]]; do
  sleep 2s
  queryCount=$(runCountQuery)
  num_retries=$((num_retries + 1))
  if [[ ! -z ${DEBUG} ]]; then
    echo -e "Current query count:\n$queryCount"
  fi
done

if [[ num_retries -eq ${MAX_RETRIES} ]]; then
    echo "Failed to obtain proper query count within ${MAX_RETRIES} retries, exiting"
    exit 1
fi
