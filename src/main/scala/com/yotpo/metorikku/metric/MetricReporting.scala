...
class MetricReporting {
    ...
    reportLagTimeColumn match {
      case Some(timeColumn) => {
        ...
        try {
          reportLagTimeColumnUnits match {
            case Some(units) =>
              TimeUnit.valueOf(units) match {
                case TimeUnit.MILLISECONDS =>
                  TimeUnit.MILLISECONDS.toMillis(
                    dataFrame.agg({ timeColumn -> "max" }).collect()(0).getLong(0)
                  )
                ...
                case _ =>
                  throw MetorikkuException("Unsupported time unit type " + TimeUnit.valueOf(units))
              }
          ...
          case e: ClassCastException =>
            throw new ClassCastException(
              s"Lag instrumentation column -${timeColumn} " +
                s"cannot be cast to spark.sql.Timestamp or spark.sql.Long"
            )
          case e: IllegalArgumentException =>
            throw new MetorikkuWriteFailedException(
      ...
      case _ =>
        throw MetorikkuWriteFailedException(
          "Failed to report lag time, reportLagTimeColumn is not defined"
        )
    }
  }
  ...
  def reportLagTime(
    ...
    dataFrame.isEmpty match {
      case false => {
        val maxDataframeTime =
          getMaxDataframeTime(dataFrame, reportLagTimeColumn, reportLagTimeColumnUnits)
