...
case class StreamingWritingConfiguration(
    dataFrame: DataFrame,
    outputConfig: Output,
    writers: ListBuffer[Writer] = ListBuffer.empty
)
...
case class Metric(configuration: Configuration, metricDir: Option[File], metricName: String) {
  ...
  def calculate(job: Job): Unit = {
    ...
    val dqConfigurator =
      new DeequFactory(log, job.config.failedDFLocationPrefix, job.config.ignoreDeequValidations)
  ...
  private def writeStream(
    ...
    writerConfig.writers.size match {
      case size if size == 1 =>
        writerConfig.writers.foreach(writer =>
          writer.writeStream(writerConfig.dataFrame, streamingConfig)
        )
      case size if size > 1 =>
        log.error(
          "Found multiple outputs for a streaming source without using the batch mode, " +
  ...
  private def repartition(outputConfig: Output, dataFrame: DataFrame): DataFrame = {
    ...
    val deprecatedRepartition = Option(outputConfig.outputOptions)
      .getOrElse(Map())
      .get("repartition")
      .asInstanceOf[Option[Int]]
    val deprecatedCoalesce = Option(outputConfig.outputOptions)
      .getOrElse(Map())
      .get("coalesce")
    ...
    (
      ...
      outputConfig.repartition.orElse(deprecatedRepartition)
    ) match {
      ...
      case Some(output) => {
        ...
        output.foreach(outputConfig => {
          ...
          outputOptions.get("protectFromEmptyOutput").asInstanceOf[Option[Boolean]] match {
            case Some(true) => {
              if (dataFrame.head(1).isEmpty) {
                throw MetorikkuWriteFailedException(
                  s"Abort writing dataframe: ${dataFrameName}, " +
          ...
          if (dataFrame.isStreaming) {
            val streamingWriterConfig = streamingWriterList.getOrElse(
              dataFrameName,
              StreamingWriting(StreamingWritingConfiguration(dataFrame, outputConfig))
            )
        ...
        for ((dataFrameName, streamingConfig) <- streamingWriterList)
          writeStream(
            dataFrameName,
            streamingConfig.streamingWritingConfiguration,
            job.config.streaming,
            job.instrumentationClient
          )
