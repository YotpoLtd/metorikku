...
case class Sql(
  ...
  private def checkpointDataframe(
      dataFrameName: String,
      df: DataFrame,
      checkpoint: Option[Boolean]
  ): Dataset[Row] = {
    checkpoint match {
      case Some(true) =>
        df.sparkSession.sparkContext.getCheckpointDir match {
          case None => {
            log.warn(
              s"Did not commit checkpoint for data frame ${dataFrameName}. No checkpoint path was specified"
            )
            df
          }
  ...
  private def runDQValidation(
      session: SparkSession,
      dfName: String,
      dqDef: Option[DataQualityCheckList]
  ): Unit = {
    dqDef match {
