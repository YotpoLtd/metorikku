...
class HasSize(level: Option[String], size: String, operator: String)
    extends Operator(level = level) {

  override def getCheck(level: String): Check = {
    new Check(getLevel(level), "Size check for data frame size: %s".format(size))
      .hasSize(Evaluator().dqAssertion(operator, size.toLong))
  }
}
