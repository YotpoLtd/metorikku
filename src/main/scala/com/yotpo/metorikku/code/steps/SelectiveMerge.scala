...
object SelectiveMerge {
  private val message =
    "You need to send 3 parameters with the names of the dataframes to merge and the key(s) to merge on" +
  ...
  def logOverrides(df1: DataFrame, df2: DataFrame, joinKeys: Seq[String]): Unit = {
    ...
    val overridenColumns =
      df2SchemaTitles.filter(p => df1SchemaTitles.contains(p) && !joinKeys.contains(p))
  ...
  def outerJoinWithAliases(df1: DataFrame, df2: DataFrame, joinKeys: Seq[String]): DataFrame = {
    ...
    val columnsRenamed = columns.map(column =>
      if (joinKeys.contains(s"$column")) s"$column" else s"$colRenamePrefix$column"
    )
