...
object LoadIfExists {
  val message =
    "You need to send 2 parameters with the names of a df and a name of a table to try to load: dfName, tableName"
    ...
    params.get match {
      case LoadIfExistsInputMatcher(dfName, tableName) => {
        ...
        if (ss.catalog.tableExists(tableName)) {
          ...
          ss.createDataFrame(ss.sparkContext.emptyRDD[Row], df.schema)
