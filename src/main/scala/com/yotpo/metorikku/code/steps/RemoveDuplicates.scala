...
object RemoveDuplicates {
  ...
  val missingOptionalParameterMsg =
    s"Didn't supply index columns (using '${columnsParameterName}' parameter), so comparing entire row"
    ...
    params match {
      case Some(parameters) => {
        ...
        val getRowColumnNames = () => {
          LogManager
            .getLogger(RemoveDuplicates.getClass.getCanonicalName)
            .warn(missingOptionalParameterMsg)
        ...
        ss.table(table)
          .dropDuplicates(columnNames.split(","))
          .createOrReplaceTempView(dataFrameName)
      }
