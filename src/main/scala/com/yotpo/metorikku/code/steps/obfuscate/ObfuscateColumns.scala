...
object ObfuscateColumns {
  ...
  private def parse(params: Option[Map[String, String]]): (Array[String], String, String) = {
    params.get match {
      case ObfuscateColumnsInputMatcher(columns, delimiter, value, table) =>
        (columns.split(delimiter), value, table)
      case _ =>
  ...
  def obfuscateColumns(df: DataFrame, columns: Array[String], value: String): DataFrame = {
    ...
    if (columnsNotInOriginalSchema.nonEmpty) {
      throw ColumnsNotPartOfOriginalSchemaException(columnsNotInOriginalSchema)
    }
    ...
    // scalastyle:off null
    val transformPreservingNull = (colName: String) =>
      when(col(colName).isNotNull, transform(colName)).otherwise(lit(null))
    // scalastyle:on null

    columns.foldLeft(df)((dfAcc, column) =>
      dfAcc.withColumn(column, transformPreservingNull(column))
    )
  }
