...
case class Tester(config: TesterConfig) {
  ...
  def run(): Unit = {
    ...
    initializeInputTables(
      job.sparkSession.catalog,
      job.sparkSession,
      config.test.mocks
    )
    ...
    metricConfig.metrics match {
      case Some(metrics) =>
        metrics.foreach(metric => {
          val metricSet = new MetricSet(metric, write = false)
          metricSet.run(job)
    ...
    if (!errors.isEmpty) {
      throw MetorikkuTesterTestFailedException(
        "Test failed:\n" + errors.mkString("\n")
      )
    } else {
  ...
  private def createMetorikkuConfigFromTestSettings(): Configuration = {
    ...
    Configuration(
      ...
      None,
      None,
      Option(config.preview > 0),
      None,
      None,
      None,
      Option(config.preview),
      None,
      None,
      None,
      None,
  ...
  private def getMockFilesForStreamingInputs(
      ...
      testDir: File
  ): Option[Map[String, Input]] = {
    mocks match {
      case Some(mockList) =>
        Option(
          mockList
            .filter(_.streaming.contains(true))
            .map(mock => {
              mock.name -> {
                val fileInput =
                  com.yotpo.metorikku.configuration.job.input.File(
                ...
                Input(
                  Option(new StreamMockInput(fileInput)),
                  None,
                  None,
                  None,
                  None,
  ...
  private def extractTableContents(
  ...
  private def compareActualToExpected(metricName: String): Array[String] = {
    ...
    val (metricExpectedTests, configuredKeys) =
      (config.test.tests, config.test.keys)
    ...
    metricExpectedTests.keys.foreach(tableName => {
      val actualResultsDf = extractTableContents(
        job.sparkSession,
        tableName,
        config.test.outputMode.get
      )
      val (expectedResults, actualResults) = (
        metricExpectedTests(tableName),
        TestUtil.getRowsFromDf(actualResultsDf)
      )
      val (expectedResultsObjects, actualResultsObjects) =
        (EnrichedRows(expectedResults), EnrichedRows(actualResults))
      val tableNameToAllExpectedColumns =
        metricExpectedTests.mapValues(v => v.head.keys.toList)
      val allExpectedColumns =
        tableNameToAllExpectedColumns.getOrElse(tableName, List[String]())
      val (isConfiguredKeysValid, keys) = getConfiguredKeysValidToTableKeys(
        configuredKeys,
        tableName,
        allExpectedColumns,
        tableName
      ...
      if (!isConfiguredKeysValid) {
        return getInvalidKeysNonExistingErrors(
          allExpectedColumns,
          keys,
          tableName
        )
      ...
      val (expectedKeys, actualKeys) = (
        ...
        keyColumns.getKeysMapFromDF(actualResultsDf)
      )
      val (expectedResultsDuplications, actualResultsDuplications) = (
        TestUtil.getDuplicatedRowToIndexes(expectedKeys),
        TestUtil.getDuplicatedRowToIndexes(actualKeys)
      )
      val colToMaxLengthValMap = TestUtil.getColToMaxLengthValue(
        expectedResults ++ actualResults
      ) // alignment per column should be same width for all tables
      val (printableExpectedResults, printableActualResults) = (
        expectedResultsObjects.addAlignmentRow(colToMaxLengthValMap),
        actualResultsObjects.addAlignmentRow(colToMaxLengthValMap)
      )
      val tableErrorDataArr: Array[ErrorMessage] =
        expectedResultsDuplications.nonEmpty || actualResultsDuplications.nonEmpty match {
          case true =>
            Array[ErrorMessage](new DuplicatedHeaderErrorMessage()) ++
              ErrorMessage.getErrorMessagesByDuplications(
                ResultsType.expected,
                expectedResultsDuplications,
                printableExpectedResults,
                tableName,
                keyColumns
              ) ++
              ErrorMessage.getErrorMessagesByDuplications(
                ResultsType.actual,
                actualResultsDuplications,
          ...
          case _ =>
            ...
            if (
              expectedKeys
                .sortWith(sorter.sortStringRows)
                .deep != actualKeys.sortWith(sorter.sortStringRows).deep
            ) {
              val (expErrorIndexes, actErrorIndexes) =
                compareKeys(expectedKeys, actualKeys)
              ErrorMessage.getErrorMessageByMismatchedKeys(
                printableExpectedResults,
                printableActualResults,
                expErrorIndexes,
                actErrorIndexes,
              ...
              ErrorMessage.getErrorMessagesByMismatchedAllCols(
                tableKeys,
                printableExpectedResults,
                printableActualResults,
                job.sparkSession,
      ...
      if (tableErrorDataArr.nonEmpty) {
        ...
        logAllResultsFailure(
          printableExpectedResults,
          printableActualResults,
          tableName
        )
        log.warn(
          s"******************************  Errors found in ${tableName}  **********************************"
        )
  ...
  ): Map[String, List[InvalidSchemaData]] = {
  ...
  private def getInvalidSchemaErrors(
      invalidSchemaMap: Map[String, List[InvalidSchemaData]]
  ): Array[String] = {
    Array[String](
      new InvalidSchemaResultsErrorMessage(invalidSchemaMap).toString
    )
  }

  private def getConfiguredKeysValidToTableKeys(
      configuredKeys: Option[Map[String, List[String]]],
      tableName: String,
      allExpectedColumns: List[String],
      metricName: String
  ) = {
    ...
    val tableConfiguredKeys =
      getConfiguredKeysByTableName(configuredKeys, tableName)
    tableConfiguredKeys match {
      case Some(configuredKeys) => // defined
        getInvalidConfiguredKeysTable(
          configuredKeys,
          allExpectedColumns
        ) match {
          ...
          case _ =>
            log.info(
              s"[$metricName - $tableName]: Configured key columns for ${tableName}: [${configuredKeys
                  .mkString(", ")}]"
            )
      ...
      case _ =>
        log.warn(
          s"[$metricName - $tableName]: Hint: Define key columns for ${tableName} for better performance"
        )
        true -> allExpectedColumns // undefined keys
    }
  }

  private def getInvalidKeysNonExistingErrors(
      allColsKeys: List[String],
      invalidKeys: List[String],
      tableName: String
  ) = {
    Array(
      new InvalidKeysNonExistingErrorMessage(
        tableName,
        invalidKeys,
        allColsKeys
      ).toString()
  ...
  private def logAllResultsFailure(
      sortedExpectedRows: EnrichedRows,
      sortedActualResults: EnrichedRows,
      tableName: String
  ): Unit = {
    log.warn(
      "**************************************************************************"
  ...
  private def getConfiguredKeysByTableName(
      configuredKeys: Option[Map[String, List[String]]],
      tableName: String
  ) = {
  ...
  private def getInvalidConfiguredKeysTable(
      configuredKeys: List[String],
      allKeys: List[String]
  ): Option[List[String]] = {
    val invalidKeys =
      configuredKeys.filter(configuredKey => !allKeys.contains(configuredKey))
  ...
  private def logAllResultsByType(
      resultsType: ResultsType.Value,
      enrichedRows: EnrichedRows,
      allExpectedColumns: Iterable[String],
      tableName: String
  ) = {
    log.warn(
      s"**************************  $tableName $resultsType results  ****************************"
    )
    val df = enrichedRows.toDF(
      resultsType,
      allExpectedColumns.toList,
      job.sparkSession
    )
    log.warn(
      TestUtil.dfToString(
        TestUtil
          .replaceColVal(df, "row_number", enrichedRows.size.toString, "  "),
        enrichedRows.size,
        truncate = false
      )
    )
  ...
  private def compareKeys(
      expRowKeyList: Array[Map[String, String]],
      actualRowKeysList: Array[Map[String, String]]
  ): (List[Int], List[Int]) = {
    val expectedMismatchedIndexes = getUnmatchedKeysIndexes(
      expRowKeyList,
      actualRowKeysList,
      ResultsType.expected
    )
    val actualMismatchedIndexes = getUnmatchedKeysIndexes(
      actualRowKeysList,
      expRowKeyList,
      ResultsType.actual
    )
  ...
  private def getUnmatchedKeysIndexes(
      expRowKeys: Array[Map[String, String]],
      actualRowKeys: Array[Map[String, String]],
      resType: ResultsType.Value
  ): List[Int] = {
  ...
  private def initializeInputTables(
      catalog: Catalog,
      sparkSession: SparkSession,
      mocks: Option[List[Mock]]
  ) = {
      ...
      case Some(mocks) =>
        mocks
          ...
          .foreach(mock => {
            ...
            val path = s"%s/%s"
              .format(config.basePath.toPath.toAbsolutePath.toString, mock.path)
            val createStatement =
              s"CREATE TABLE ${tableInfo.database}.${tableInfo.tableName} USING ${fileFormat} LOCATION '${path}'"
