...
case class MismatchData(
    expectedIndex: Int,
    actualIndex: Int,
    mismatchingCols: List[String],
    mismatchingVals: List[String],
    keyDataStr: String
)

case class InvalidSchemaData(
    rowIndex: Int,
    invalidColumnsMissing: List[String],
    invalidColumnsUnexpected: List[String]
)
...
class InvalidKeysNonExistingErrorMessage(
    tableName: String,
    invalidCols: List[String],
    allCols: List[String]
...
class InvalidSchemaResultsErrorMessage(
    tableToInvalidSchemaData: Map[String, List[InvalidSchemaData]]
) extends ErrorMessage {

  override def logError(sparkSession: Option[SparkSession] = None): Unit = {}

  override def toString: String = {
    val invalidTableSchemaMessage = tableToInvalidSchemaData.map {
      case (tableName, listOfSchemaErrData) =>
          ...
          listOfSchemaErrData
            .map(schemaErrData => {
              ...
              val invalidColumnsMissing = schemaErrData.invalidColumnsMissing.nonEmpty match {
                case true =>
                  s"\tExpected row number ${schemaErrData.rowIndex} is missing the following expected columns: " +
...
class DuplicationsErrorMessage(
    resultType: ResultsType.Value,
    duplicatedRowsToIndexes: Map[Map[String, String], List[Int]],
    results: Option[EnrichedRows],
    tableName: String,
  ...
  override def logError(sparkSession: Option[SparkSession]): Unit = {
    ...
    log.warn(
      s"*****************  $tableName $resultType results with Duplications  *******************"
    )
    ...
    log.warn(
      TestUtil.dfToString(
        TestUtil.replaceColVal(dfWithId, "row_number", results.get.size().toString, "  "),
        subExpectedError.size,
        truncate = false
      )
    )
  }
  ...
  override def toString: String = {
    s"$tableName Duplications - ${duplicatedRowsToIndexes
        .map { case (row, indexes) =>
          s"The key [${keyColumns.getRowKeyStr(row)}] was found in the ${resultType} results rows: " +
            s"${indexes.map(_ + 1).sortWith(_ < _).mkString(", ")}"
        }
        .mkString("\n")}"
  }
}

class MismatchedKeyResultsErrorMessage(
    expectedErrorIndexes: List[Int],
    actualErrorIndexes: List[Int],
    expectedResults: EnrichedRows,
    actualResults: EnrichedRows,
    keyColumns: KeyColumns,
    tableName: String
) extends ErrorMessage {
  ...
  override def logError(sparkSession: Option[SparkSession] = None): Unit = {
    ...
    EnrichedRows.logSubtableErrors(
      expectedResults,
      actualResults,
      expectedErrorIndexes :+ alignmentRowIndexExpected,
      actualErrorIndexes :+ alignmentRowIndexActual,
      true,
      sparkSession.get,
      tableName
    )
  ...
  override def toString: String = {
    expectedErrorIndexes
      .map(errorRowindex => {
        val keyToOutput =
          keyColumns.getRowKeyStr(expectedResults.getEnrichedRowByIndex(errorRowindex).getRow())
        s"Error: Missing expected " +
          s"row with the key [${keyToOutput}] - (expected row_number = ${errorRowindex + 1})"
      })
      .mkString(",\n") + "\n\n" +
      actualErrorIndexes
        .map(errorRowindex => {
          val keyToOutput =
            keyColumns.getRowKeyStr(actualResults.getEnrichedRowByIndex(errorRowindex).getRow)
          s"Error: Got unexpected result - didn't expect to find " +
            s"a row with the key [${keyToOutput}] (printed row_number in actual results = ${errorRowindex + 1})"
        })
        .mkString(",\n")
  }
}

class MismatchedResultsAllColsErrorMsg(
    expectedResults: EnrichedRows,
    actualResults: EnrichedRows,
    mismatchData: List[MismatchData],
    tableName: String
) extends ErrorMessage {
  ...
  override def logError(sparkSession: Option[SparkSession] = None): Unit = {
    ...
    EnrichedRows.logSubtableErrors(
      expectedResults,
      actualResults,
      mismatchData.map(_.expectedIndex) :+ alignmentRowIndexExpected,
      mismatchData.map(_.actualIndex) :+ alignmentRowIndexActual,
      true,
      sparkSession.get,
      tableName
    )
  ...
  override def toString(): String = mismatchData
    .map(errData => {
...
class MismatchedResultsKeysErrMsgMock(
    errorIndexes: (ResultsType.Value, Int),
...
class MismatchedResultsColsErrMsgMock(
    rowKeyStr: String,
...
object ErrorMessage {
  ...
  def getErrorMessagesByDuplications(
      resType: ResultsType.Value,
      duplicatedRowToIndexes: Map[Map[String, String], List[Int]],
    ...
    if (duplicatedRowToIndexes.nonEmpty) {
      Array[ErrorMessage](
        new DuplicationsErrorMessage(
          resType,
          duplicatedRowToIndexes,
          Option(results),
          tableName,
          keyColumns
        )
  ...
  def getErrorMessagesByMismatchedAllCols(
      tableKeys: List[String],
      expectedEnrichedRows: EnrichedRows,
      actualEnrichedRows: EnrichedRows,
      sparkSession: SparkSession,
      tableName: String
  ): Array[ErrorMessage] = {
    val sorter = TesterSortData(tableKeys)
    val (sortedExpectedEnrichedRows, sortedActualEnrichedRows) = (
      expectedEnrichedRows.sortWith(sorter.sortEnrichedRows),
      actualEnrichedRows.sortWith(sorter.sortEnrichedRows)
    )
    sortedExpectedEnrichedRows.zipWithIndex
      .flatMap { case (expectedResult, sortedIndex) =>
        ...
        if (mismatchingCols.nonEmpty) {
          getMismatchedAllColsErrorMsg(
            List[(Int, Int)]() :+ (expectedIndex, actualIndex),
            expectedEnrichedRows,
            actualEnrichedRows,
            tableKeys,
            sparkSession,
            tableName
  ...
  def getMismatchedAllColsErrorMsg(
    ...
    val mismatchDataArr = expectedMismatchedActualIndexesMap.map {
      case (expIndex, actIndex) => {
        ...
        val mismatchingVals =
          TestUtil.getMismatchedVals(expRow.getRow(), actRow.getRow(), mismatchingCols).toList
    ...
    Array[ErrorMessage](
      new MismatchedResultsAllColsErrorMsg(
        expectedResults,
        actualResults,
        mismatchDataArr,
        tableName
  ...
  def getErrorMessageByMismatchedKeys(
      expectedResults: EnrichedRows,
      actualResults: EnrichedRows,
      expErrorIndexes: List[Int],
      actErrorIndexes: List[Int],
      keyColumns: KeyColumns,
      tableName: String
  ): Array[ErrorMessage] = {
    Array[ErrorMessage](
      new MismatchedKeyResultsErrorMessage(
        expErrorIndexes,
        actErrorIndexes,
        expectedResults,
        actualResults,
