...
case class KafkaInput(
  ...
  def read(sparkSession: SparkSession): DataFrame = {
    ...
    schemaRegistryUrl match {
      case Some(url) => {
        val schemaRegistryMap =
          createSchemaRegistryConfig(url, schemaSubject.getOrElse(topic.get), schemaId)
        kafkaDataFrame
          .select(from_confluent_avro(col("value"), schemaRegistryMap) as "result")
          .select("result.*")
      }
  ...
  private def createSchemaRegistryConfig(
      schemaRegistryUrl: String,
      schemaRegistryTopic: String,
      schemaId: Option[String]
  ): Map[String, String] = {
