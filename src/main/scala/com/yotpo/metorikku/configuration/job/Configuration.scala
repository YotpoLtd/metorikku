...
case class Configuration(
    ...
    var configFile: Option[String] // Store the file path
) {
  ...
  failedDFLocationPrefix = failedDFLocationPrefix.orElse(
    getEnvProperties().get("CONFIG_FAILED_DF_PATH_PREFIX")
  )
  checkpointStorageLocation = checkpointStorageLocation.orElse(
    getEnvProperties().get("CONFIG_CHECKPOINT_STORAGE_LOCATION")
  )
  quoteSparkVariables = Option(quoteSparkVariables.getOrElse(true))

  def getReaders: Seq[Reader] = inputs
    .getOrElse(Map())
    .map { case (name, input) =>
