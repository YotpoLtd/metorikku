...
case class FileDateRange(
  ...
  override def getReader(name: String): Reader =
    FilesInput(name, dateRange.replace(template), options, schemaPath, format)
}
...
class DateRange(
  ...
  require(
    ...
    s"startDate:${startDate} must be earlier than endDate:${endDate}"
  )

  /** Generate a sequence of strings
    *
    * @param templateString
    *   example: "/analytics/user_agg/%s/"
    * @return
    */
