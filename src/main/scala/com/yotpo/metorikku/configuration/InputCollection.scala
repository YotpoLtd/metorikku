...
import com.yotpo.metorikku.configuration.input.{File, FileDateRange, JDBC, Kafka}
...
case class InputCollection(@JsonProperty("file") fileInput: Option[File],
  ...
  def getInput: Input = fileInput.getOrElse(fileDateRangeInput.getOrElse(jdbcInput.getOrElse(kafkaInput.getOrElse(Empty()))))
}

object InputCollection {
  def apply(): InputCollection = new InputCollection(None, None, None, None)
}
