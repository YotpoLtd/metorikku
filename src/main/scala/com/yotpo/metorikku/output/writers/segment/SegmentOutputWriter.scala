...
class SegmentOutputWriter(
    props: Map[String, String],
    segmentOutputConf: Option[Segment],
    instrumentationFactory: InstrumentationFactory
) extends Writer {

  case class SegmentOutputProperties(
      eventType: String,
      keyColumn: String,
      eventName: String,
      sleep: Int,
  ...
  if (eventType == "identify") {
    setMandatoryArguments("keyColumn")
  } else {
    setMandatoryArguments("keyColumn", "eventName")
  }

  val segmentOutputOptions =
    SegmentOutputProperties(eventType, keyColumn, eventName, sleep, batchSize)
        ...
        segmentOutputOptions.batchSize match {
          ...
          case _ =>
            dataFrame
              ...
              .cache()
              .toLocalIterator
              .grouped(segmentOutputOptions.batchSize)
              .foreach(part => {
                writeEvents(
                  segmentApiKey,
                  part.map(row_element => { row_element.get(0).toString }).iterator
                )
              })
        }
      case None =>
      ...
      try {
        ...
        segmentOutputOptions.eventType match {
          case "track" =>
            analytics.enqueue(
              TrackMessage
                .builder(segmentOutputOptions.eventName)
                .userId(userId)
