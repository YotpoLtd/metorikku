...
class InstrumentationOutputWriter(
  ...
  override def write(dataFrame: DataFrame): Unit = {
    ...
    val indexOfValCol =
      valueColumnProperty.flatMap(col => Option(dataFrame.schema.fieldNames.indexOf(col)))
    val indexOfTimeCol =
      timeColumnProperty.flatMap(col => Option(dataFrame.schema.fieldNames.indexOf(col)))

    log.info(
      s"Starting to write Instrumentation of data frame: $dataFrameName on metric: $metricName"
    )
      ...
      p.foreach(row => {
        try {
          ...
          val tags = Map("metric" -> metricName, "dataframe" -> dataFrameName) ++
            columns
              .filter { case (column, index) =>
                index != actualIndexOfValCol && (!indexOfTimeCol.isDefined || index != indexOfTimeCol.get)
              }
          ...
          case ex: Throwable =>
            throw MetorikkuWriteFailedException(
              s"failed to write instrumentation on data frame: $dataFrameName " +
                s"for row: ${row.toString()}",
              ex
            )
        }
      })
