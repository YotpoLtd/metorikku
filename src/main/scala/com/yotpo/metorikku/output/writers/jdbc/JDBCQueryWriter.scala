...
class JDBCQueryWriter(props: Map[String, String], config: Option[JDBC]) extends Writer {
  ...
  case class JDBCQueryProperties(
      query: String,
      maxBatchSize: Int,
  ...
  val options = JDBCQueryProperties(
    props("query"),
  ...
  override def write(dataFrame: DataFrame): Unit = {
    config match {
      case Some(config) =>
        alignPartitions(dataFrame, options.minPartitions, options.maxPartitions).foreachPartition {
          partition: Iterator[Row] =>
            val conn =
              DriverManager.getConnection(config.connectionUrl, config.user, config.password)
            val stmt = conn.prepareStatement(options.query)

            partition
              .grouped(options.maxBatchSize)
              .foreach(batch => {
                batch.foreach(row => {
                  for (i <- 1 to row.size) {
