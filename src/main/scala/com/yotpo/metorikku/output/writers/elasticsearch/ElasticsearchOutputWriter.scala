...
class ElasticsearchOutputWriter(props: Map[String, Object], elasticsearchOutputConf: Elasticsearch)
    extends Writer {

  case class ElasticsearchOutputProperties(
  ...
  private def save(writer: DataFrameWriter[_]) = {
    ...
    val indexName =
      (
        elasticsearchOutputProperties.resource,
        elasticsearchOutputProperties.addTimestampToIndex
      ) match {
        case (Some(resource), Some(true)) => resource + "-" + DateTime.now.getMillis.toString
        case (Some(resource), _)          => resource
        case (None, _) =>
          throw MetorikkuException(s"Target index must be specified under 'resource' option")
      }
