...
class MetricReporterTester extends AnyFunSuite with BeforeAndAfterEach {
  ...
  test("Test getMaxDataframeTime") {
    ...
    val df = sparkSession.read
      ...
      .option("header", "true")
      .schema(schema)
      .load("src/test/configurations/mocks/ratings_time.csv")
  ...
  test("Test getMaxDataframeTime FAILs with invalid reportLagTimeColumn Units specified") {
    ...
    assert(
      thrown.getMessage.startsWith(
        "Some(HOUR) is not a legal argument for units, use one of the following: [SECONDS,MILLISECONDS]"
      )
    )
  }
  ...
  test("Test getMaxDataframeTime FAILs reportLagTimeColumn is not defined") {
    ...
    assert(
      thrown.getMessage.startsWith("Failed to report lag time, reportLagTimeColumn is not defined")
    )
  }
