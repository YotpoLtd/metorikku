...
class HasUniquenessTest extends FunSuite with BeforeAndAfterEach {
  ...
  private def valideHasUniquenessOverDf(employeeData: Seq[(String, Int, String, Int, Int)], columns: Seq[String],
                                        assertion: Double => Boolean, level: String) = {
    val sqlContext = sparkSession.sqlContext
    val hasUniquenessCheck = new HasUniqueness(level = Some(level),columns = columns, assertion=assertion)
  ...
  test("has_uniqueness on a non-unique field with level error should raise exception") {
    ...
    val thrown = intercept[Exception] {
      valideHasUniquenessOverDf(employeeData, Seq("id", "name"), Check.IsOne, level)
    }
