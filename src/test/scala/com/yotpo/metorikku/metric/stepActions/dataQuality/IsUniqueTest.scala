...
class IsUniqueTest extends FunSuite with BeforeAndAfterEach {
  ...
  test("is_unique on a unique field with level error should not raise exception", UnsupportedInCurrentVersion) {
    val employeeData = Seq(
      ("James", 1, 11, 111, 1111),
      ("Maria", 2, 22, 222, 2222)
    )
