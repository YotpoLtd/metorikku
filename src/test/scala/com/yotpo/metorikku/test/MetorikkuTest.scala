...
class MetorikkuTest extends AnyFunSuite with BeforeAndAfterAll {
  ...
  test("Test Metorikku should load a table and filter") {
    Metorikku.main(
      Array(
        "-c",
        "src/test/scala/com/yotpo/metorikku/test/metorikku-test-config.yaml"
      )
    )

    assert(
      new File(
        "src/test/out/metric_test/metric/testOutput/._SUCCESS.crc"
      ).exists
    )
    assert(
      new File(
        "src/test/out/metric_test/metric/filteredOutput/._SUCCESS.crc"
      ).exists
    )

    assertOnOutput(
      "src/test/out/metric_test/metric",
  ...
  test("Test Metorikku should Fail on invalid metics") {
    val thrown = intercept[FileNotFoundException] {
      Metorikku.main(
        Array(
          "-c",
          "src/test/scala/com/yotpo/metorikku/test/metorikku-test-config-invalid-metrics.yaml"
        )
      )
    }
  ...
  test("Test Metorikku should Fail on invalid inputs path") {
    val thrown = intercept[Exception] {
      Metorikku.main(
        Array(
          "-c",
          "src/test/scala/com/yotpo/metorikku/test/metorikku-test-config-invalid-inputs.yaml"
        )
      )
    }
  ...
  test("Test Metorikku should Fail on invalid Writer") {
    assertThrows[MetorikkuInvalidMetricFileException] {
      Metorikku.main(
        Array(
          "-c",
          "src/test/scala/com/yotpo/metorikku/test/metorikku-test-config-invalid-writer.yaml"
        )
      )
    }
  }
...
//  }
  ...
  test(
    "Test Metorikku should not fail on invalid query when ignoreOnFailures is set to true"
  ) {
    Metorikku.main(
      Array(
        "-c",
        "src/test/scala/com/yotpo/metorikku/test/metorikku-test-config-ignore-step.yaml"
      )
    )
  ...
  private def assertOnOutput(
      outputFolder: String,
      tables: Seq[String],
      assertions: (SparkSession) => Unit
  ): Unit = {
    val sparkSession = SparkSession.builder.getOrCreate()
    try {

      tables.map(table => {
        new File(outputFolder + s"/${table}").listFiles
  ...
  test("Test Metorikku should Fail on invalid keys configuration") {
    ...
    val headerExpectedMsg =
      new InvalidKeysNonExistingErrorMessage(tableName, undefinedCols, allKeys)
        .toString()
  ...
  test("Test Metorikku should Fail on inconsistent schema extra columns") {
    ...
    val mismatchedSchemaIndexes = List[InvalidSchemaData](
      InvalidSchemaData(1, List[String](), List[String]("bad_col")),
      InvalidSchemaData(
        2,
        List[String](),
        List[String]("bad_column", "bad_column2")
      )
    )
    invalidSchema += ("accountsDf" -> mismatchedSchemaIndexes)
    val expectedMsg =
      new InvalidSchemaResultsErrorMessage(invalidSchema).toString
  ...
  test("Test Metorikku should Fail on inconsistent schema missing columns") {
    ...
    val mismatchedSchemaIndexes = List[InvalidSchemaData](
      InvalidSchemaData(
        1,
        List[String]("extra_col1", "extra_col2", "extra_col3", "extra_col4"),
        List[String]()
      ),
      InvalidSchemaData(
        2,
...
//  }
  ...
  test("Test Metorikku should Fail on duplicated keyed actual results") {
    ...
    val expectedMsg = new DuplicationsErrorMessage(
      ...
      None,
      "accountsDf",
      KeyColumns(List[String]("app_key"))
    ).toString
    assert(thrown.getMessage.contains(expectedMsg))
  }

  test(
    "Test Metorikku should Fail on duplicated actual anscad expected results"
  ) {
    val thrown = intercept[Exception] {
      val test = parseConfigurationFile(
  ...
  test(
    "Test Metorikku should Fail on mismatch in key columns with empty values"
  ) {
    var definedKeys = List[String]()
    val thrown = intercept[Exception] {
      val test = parseConfigurationFile(
  ...
  test(
    "Test Metorikku should keep order of unsorted expected results with keys"
  ) {
    var definedKeys = List[String]()
    val thrown = intercept[Exception] {
      val test = parseConfigurationFile(
  ...
  test(
    "Test Metorikku should Fail on multiple df errors - duplicated keyed actual results and mismatch in all columns with keys"
  ) {
    var definedKeys = List[String]()
    val thrown = intercept[Exception] {
      val test = parseConfigurationFile(
  ...
  test(
    "Test Metorikku should Fail when results mismatch and more actual columns than expected"
  ) {
    var definedKeys = List[String]()
    val thrown = intercept[Exception] {
      val test = parseConfigurationFile(
  ...
  test("Test Metorikku should Fail when results unsorted and mismatched") {
    ...
    val thrown = intercept[Exception] {
      val test = parseConfigurationFile(
        "src/test/configurations/metorikku-tester-test-unsorted-columns-failure.json"
      )
  ...
  test(
    "Test Metorikku should fail on writing an empty df, when protectFromEmptyOutput is enabled"
  ) {
    val thrown = intercept[Exception] {
      Metorikku.main(
        Array(
          "-c",
          "src/test/scala/com/yotpo/metorikku/test/metorikku-test-empty-output-protection-config.yaml"
        )
      )
    }
    assert(
      thrown.getMessage.startsWith(
        "Abort writing dataframe: resultDfEmpty, empty dataframe output is not allowed according to configuration"
      )
    )
  }

  test(
    "Test Metorikku should support multiple catalog databases - JSONL inputs"
  ) {
    var definedKeys = List[String]()
    val test = parseConfigurationFile(
  ...
  test(
    "Test Metorikku should support multiple catalog databases - CSV inputs"
  ) {
    var definedKeys = List[String]()
    val test = parseConfigurationFile(
  ...
  test(
    "Test Metorikku should load a metric file from same path as job and filter"
  ) {
    Metorikku.main(
      Array(
        "-c",
        "src/test/calculations/valid_metric_test_local/metorikku-test-config-local.yaml"
      )
    )
  ...
  private def assertMismatchActual(
      definedKeys: List[String],
      thrownMsg: String,
  ...
  private def assertMismatchByType(
    ...
    val expectedMsg = new MismatchedResultsKeysErrMsgMock(
      resType -> rowIndex,
      expectedRow,
      actRow,
      keyColumns
