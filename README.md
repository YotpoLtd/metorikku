...
To enable all other writers and also enable multiple outputs for a single streaming dataframe, add ```streamingBatchMode``` to your job configuration, this will enable the ```foreachBatch``` mode (only available in spark >= 2.4.0)
